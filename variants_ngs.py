...
def anotation(output):
    ...
    for fichero in vcfs:
        ...
        os.system("perl annovar/table_annovar.pl {0}/annotate/{1} annovar/humandb/ -buildver hg38 -out {0}/annotate/{1} -remove -protocol refGene,cytoBand,clinvar_20140702,exac03,avsnp147,dbnsfp30a,EUR.sites.2015_09 -operation g,r,f,f,f,f,f -nastring . -csvout -polish -xref annovar/example/gene_fullxref.txt".format(output,fichero))
        os.system("awk -f filtro_awk {0}/annotate/{1}.{2}_multianno.csv > {0}/annotate/{1}.{2}_multianno_filtrado.csv".format(output,fichero,"hg38"))
        os.system("python multianno_vcf_annot.py -i {0}/annotate/{1}.{2}_multianno_filtrado.csv -o {0}/annotate/{1}.{2}_multianno_filtrado_genot.csv -v {0}/annotate/{1}".format(output,fichero,"hg38"))
